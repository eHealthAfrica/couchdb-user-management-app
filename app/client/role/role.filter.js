...
angular.module('app.role')
  ...
  .filter('getProgramsInFacility', function () {
    return function (programs, facilityPrograms, facilityId) {
      if (!facilityId || !facilityPrograms){ return []; }
      else {
  ...
  .filter('getUserType', function () {
    return function (user) {
      if (! user.lomis_stock) { return 'Unassigned'; }
      else {
        if (_.isEmpty(user.lomis_stock.mobile) && _.isEmpty(user.lomis_stock.dashboard)) { return 'None'; }
