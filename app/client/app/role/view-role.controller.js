...
angular.module('app.role')
  .controller('ViewRoleCtrl', ['$scope', 'adminLevelService', 'facilityService', 'locationService', function ($scope, adminLevelService, facilityService, locationService) {
    ...
    vm.getRole = function () {
      if (!vm.user.lomis_stock || (_.isEmpty(vm.user.lomis_stock.mobile) && _.isEmpty(vm.user.lomis_stock.dashboard))) { return 'None' } else if (_.isEmpty(vm.user.lomis_stock.mobile)) { return 'Dashboard' } else { return 'Mobile' }
    }
    ...
    vm.getLocations = function () {
      if (vm.getRole() === 'Mobile') {
        ...
        for (var i in facilities) {
          ...
          if (facility.length > 0) {
            for (var key in vm.facilities) {
              if (vm.facilities[key]._id === facility[0]) {
                assignedFacilities += vm.facilities[key].name + ' '
              }
            }
          }
        }
        ...
        var accessItem = null
        for (var key_ in accessItems) {
          if (accessItems[key_][accessLevel]) {
            accessItem = accessItems[key_][accessLevel]
            break
          }
        }
        var assignedLocations = ''

        for (var key__ in accessItem) {
          for (var j in vm.locations) {
            if (vm.locations[j]._id === accessItem[key__]) {
